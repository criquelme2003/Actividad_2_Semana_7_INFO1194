...
public:
	ParallelGeneticAlgorithm(const ProblemInstance &instance, GAConfig config, int seed, int num_threads);
	...
	// Devuelve la mejor solución factible encontrada durante toda la evolución.
	// Si no existe ninguna, devuelve un individuo vacío (genes vacíos).
	EvaluatedIndividual get_best_feasible() const;
	bool has_feasible_solution() const;

	// Devuelve los n cromosomas de mayor fitness de la población actual.
	// Re-evalúa la población para obtener el ranking; usado por IslandModel
	// para seleccionar migrantes antes de cada ronda de migración.
...
private:
	...
	std::vector<Chromosome> generate_offspring_parallel(const std::vector<EvaluatedIndividual> &evaluated,
	                                                    int offspring_needed);
