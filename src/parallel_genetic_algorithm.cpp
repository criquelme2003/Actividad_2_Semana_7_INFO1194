...
ParallelGeneticAlgorithm::ParallelGeneticAlgorithm(const ProblemInstance &instance,
                                                   GAConfig config, const int seed,
                                                   const int num_threads)
    : instance_(instance), config_(config), evaluator_(instance, config.penalties),
      num_threads_(num_threads), master_rng_(seed) {
...
vector<Chromosome> ParallelGeneticAlgorithm::generate_offspring_parallel(
    const vector<EvaluatedIndividual> &evaluated, const int offspring_needed) {
...
#pragma omp parallel for schedule(static) num_threads(num_threads_)
    for (int p = 0; p < num_pairs; ++p) {
        ...
        auto [child_a, child_b] = crossover_one_point(evaluated[idx_a].chromosome,
...
static bool run_one_generation_impl(
    ParallelGeneticAlgorithm &self,
    std::vector<EvaluatedIndividual> (ParallelGeneticAlgorithm::*eval_fn)() const,
...
GARunResult ParallelGeneticAlgorithm::run() {
    ...
    for (int generation = 0; generation < config_.generations; ++generation) {
        ...
        if (generation == 0 ||
            evaluated.front().evaluation.fitness > result.best_by_fitness.evaluation.fitness) {
        ...
        for (const auto &ind : evaluated) {
            ...
            if (!result.has_feasible ||
                ind.evaluation.fitness > result.best_feasible.evaluation.fitness) {
        ...
        const int offspring_needed =
            static_cast<int>(population_.size()) - config_.elitism_count;
...
void ParallelGeneticAlgorithm::run_n_generations(const int n) {
    ...
    for (int g = 0; g < n; ++g) {
        ...
        for (const auto &ind : evaluated) {
            ...
            if (!has_feasible_ ||
                ind.evaluation.fitness > best_feasible_.evaluation.fitness) {
                best_feasible_ = ind;
                has_feasible_  = true;
            }
            break;
...
void ParallelGeneticAlgorithm::inject_individuals(const vector<Chromosome> &migrants) {
    ...
    const size_t replace_count =
        std::min(migrants.size(), population_.size());
...
EvaluatedIndividual ParallelGeneticAlgorithm::get_best() const {
    return best_by_fitness_;
}
